...
class ElasticCompression2D(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for step in range(0,1500):  # ; t<solver.tmax; t+=solver.dt):
            ...
            cls.nodes = nodes
            super(ElasticCompression2D, cls).setUpClass()

    def test_DisplacementPoint950_Elastic(self):
            displacementX = self.matpoints.getDispX(950)
            testValue = -1.06312665129e-06 
            self.assertAlmostEqual(displacementX,testValue)
    def test_DisplacementPoint7990_Rigid(self):
            displacementY = self.matpoints.getDispY(7990)
            testValue = -4.122372826431686e-05
