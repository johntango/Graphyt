...
psep = cellsize/2.0
# Background Grid
L = [200*cellsize,200*cellsize,0.0]
...
l0 = 0.057
h0 = 0.057
liquidLowerLeft = [10*cellsize,10*cellsize,0.0]
liquidUpperRight = [liquidLowerLeft[0]+l0,liquidLowerLeft[1]+h0,0.0]
...
wallthickness = 0.75*nodes.cellsize
for  n in range(nodes.numNodes):
    posX = nodes.getPosX(n)
    posY = nodes.getPosY(n)
    if (((posX>=L[0]-wallthickness) or (posX <= liquidLowerLeft[0]-wallthickness))):
        nodes.setBC(n,graphyt.BCTypes.grid_vx,[0])
    if (((posY>=L[1]-wallthickness) or (posY <= liquidLowerLeft[1]-wallthickness))):
        nodes.setBC(n,graphyt.BCTypes.grid_vx_vy,[0,0])
print("+++++ MODEL  INFORMATION+++++ Boundary Conditions Applied")

# /////////// Parameters Set /////////////// #
tmax = 5.0   # //(time in seconds)
is3D = False
parameters = graphyt.Parameters(tmax,is3D)
parameters.setGravity(0, -9.81, 0)
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
  ...
  #  matpoints.setRotatingBodyVel(0,wheelX,wheelY,0.0,omega)
    vtpfname='output/verificationMFN02'+str(step)+'.vtp'
    if step % 100 == 0:
        vtp.Write(vtpfname)
