...
psep = cellsize/2.0
# Background Grid
L = [0.2+4*cellsize,0.24+5*cellsize, 0.15+5*cellsize]
...
pelton = pyck.StlShape(1,'peltonCAD.stl',[0.005,0.05,0.09],2.0,[0.0,0.0,0.0],0.0)
cubeLowerLeft = [0.01,0.01,0.05]
cubeUpperRight = [0.2,0.10,0.15]
cube = pyck.Cuboid(2,cubeLowerLeft,cubeUpperRight)
# Add shapes to pack
pack.AddShape(cube)
pack.AddShape(pelton)
pack.Process()
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
    ...
    vtpfname='output/peltonScaled'+str(step)+'.vtp'
    if step % 250 == 0:
        vtp.Write(vtpfname)
