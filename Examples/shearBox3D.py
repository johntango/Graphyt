...
cubic = pyck.CubicPacker(L,psep,[0.25*cellsize,0.25*cellsize,0.25*cellsize])
pack = pyck.StructuredPack(cubic)
botBox = pyck.Cuboid(4,[0.05+7*cellsize,0.05+5*cellsize,0.05+7*cellsize],[0.05+L[0]-7*cellsize,0.05+L[1]/2 - 10*cellsize,0.05+L[2]-7*cellsize])
topBox = pyck.Cuboid(3,[0.05+7*cellsize,0.05+L[1]/2+10*cellsize,0.05+7*cellsize],[L[0]-7*cellsize+0.05,L[1]-5*cellsize+0.05,0.05+L[2]-7*cellsize])
solid_bot = pyck.Cuboid(2,[0.05+11*cellsize,0.05+9*cellsize,0.05+7*cellsize],[0.05+L[0]-11*cellsize,0.05+L[1]/2,0.05+L[2]-7*cellsize])
solid_top = pyck.Cuboid(1,[0.05+11*cellsize,0.05+L[1]/2,0.05+7*cellsize],[0.05+L[0]-11*cellsize,0.05+L[1]-9*cellsize,0.05+L[2]-7*cellsize])
...
nz = 1  + int(round((L[2] - 14*cellsize)/cellsize) )
appeture = genFracture.drazerKoplik(nx=nx,ny=nz,zeta=zeta,seed=5)
# print(appeture)
# print(appeture[1][1])
xapp = appeture[:][0]
target = find_nearest(xapp,0.04)
err = abs(xapp[0] - target) 
while (err> cellsize/2 ):
    xapp = np.roll(xapp,1,axis=0)
    err = abs(xapp[0] - target) 

# print(appeture)
for p in range(matpoints.numParticles):
    ypos = matpoints.getPosY(p)
    zpos = matpoints.getPosZ(p) - 7*cellsize
    xpos = matpoints.getPosX(p) - 7*cellsize
    cellID_x = int(round(xpos/cellsize))
    cellID_z = int(round(zpos/cellsize))
    yint =  int(0.025*appeture[cellID_x][cellID_z] + 0.4*L[1])
    if((matpoints.getObjID(p) is not 3)  and (matpoints.getObjID(p) is not 4) ):
        if(ypos > yint):
            matpoints.setObjectID(p,1)
        else:
...
for step in range(int(solver.tmax/solver.dt)):
    ...
    vtpfname='output/shearBox3D'+str(step)+'.vtp'
