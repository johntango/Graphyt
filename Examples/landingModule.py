...
# create shapes
landingModule = pyck.StlShape(1,'landingModule.STL',[-250.0,-250.0,-500.0],0.250,[0.0,0.0,0.0],0.0)
# Add shapes to pack
pack.AddShape(landingModule)
pack.Process()
...
for p in range(matpoints.numParticles):
    ...
    matpoints.setStress(p,0.0,y,0.0,0.0,0.0,0.0)
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
  ...
  #  matpoints.setRotatingBodyVel(0,wheelX,wheelY,0.0,omega)
    vtpfname='output/landingModule'+str(step)+'.vtp'
    if step % 250 == 0:
        vtp.Write(vtpfname)
