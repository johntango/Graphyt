...
flawUpperRight = [0.05+0.04445,0.01+0.0712+cellsize,2*cellsize]
...
for  n in range(nodes.numNodes):
   ...
   if (posY > 0.15):
       nodes.setBC(n,graphyt.BCTypes.grid_vx_vy,[0,-0.2])
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
    ...
    vtpfname='output/bigsingleFlawLR'+str(step)+'.vtp'
