...
#sys.path.insert(0,'/f/sjr/VTKWriter/build')
import VTKWriter
import math
## GEOMETRY ##
...
theta = 60.0
flaw_a= [0.05+(0.5*0.0762 - 0.5*0.0127*math.cos(theta*3.14159/180)),0.01+0.5*0.1524 - 0.5*0.0127*math.sin(theta*3.14159/180) - 6*cellsize,0.0]
flaw_b = [flaw_a[0]+0.0127*math.cos(theta*3.14159/180),flaw_a[1]+0.0127*math.sin(theta*3.14159/180)- 6*cellsize,0.0]
flaw_c= [flaw_b[0],flaw_b[1]+1*cellsize+0.00127,0.0]
flaw_d = [flaw_a[0],flaw_a[1]+1*cellsize+0.00127,0.0]
flaw = pyck.ConvexHull2D(0,[flaw_d,flaw_c,flaw_b,flaw_a])
roundR = 0.00127/2
x0 = 0.05 + 0.5*0.0762
y0 = 0.01 + 0.5*0.1524
c1x = flaw_a[0]
c2x = flaw_b[0]
c1y = flaw_a[1]+0.5*0.00127+cellsize
c2y = flaw_b[1]+0.5*0.00127+cellsize
round1 = pyck.Cylinder(0,[c1x,c1y,0.0],roundR,[0,0,cellsize])
round2 = pyck.Cylinder(0,[c2x,c2y,0.0],roundR,[0,0,cellsize])
...
for p in range(matpoints.numParticles):  
	...
	if(idOb == 2):
               matpoints.setVel(p,0.0,-0.40,0.0)
	if(idOb == 3):
	       matpoints.setVel(p,0.0,0.0,0.0)
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
    ...
    vtpfname='output/singleFlaw_rounded_60Deg_wRigid'+str(step)+'.vtp'
