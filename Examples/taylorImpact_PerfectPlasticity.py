...
# create shapes
impactor = pyck.Cuboid(1,[0.2,0.35,2*cellsize],[0.8,0.65,2*cellsize])
# Add shapes to pack
pack.AddShape(impactor)
...
# /////////// Parameters Set /////////////// #
tmax = 0.005  # //(time in seconds)
is3D = False
parameters = graphyt.Parameters(tmax,is3D)
parameters.setGravity(0, 0.0, 0)
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
    ...
    vtpfname='output/taylorImpactPP'+str(step)+'.vtp'
    if step % 2000 == 0:
        vtp.Write(vtpfname)
