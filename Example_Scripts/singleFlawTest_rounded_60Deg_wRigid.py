...
round1 = pyck.Cylinder(0,[c1x,c1y,0.0],roundR,[0,0,0.1*cellsize])
round2 = pyck.Cylinder(0,[c2x,c2y,0.0],roundR,[0,0,0.1*cellsize])
...
for p in range(matpoints.numParticles):  
	...
	if(idOb == 2):
               matpoints.setVel(p,0.0,-0.20,0.0)
	if(idOb == 3):
...
for step in range(int(solver.tmax/solver.dt)):  # ; t<solver.tmax; t+=solver.dt):
    ...
    vtpfname='singleFlaw_rounded_60Deg_wRigid'+str(step)+'.vtp'
    if (step % 5000 == 0):
        vtp.Write(vtpfname)
